...
import { useCreateAttendanceRuleMutation } from "../../api/AttendanceRulesApi";
...
const AddAttendanceRule = () => {
  ...
  const [createAttendanceRule, { isLoading }] = useCreateAttendanceRuleMutation();
...
const workTypeOptions = [
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await createAttendanceRule(formDataToUpload).unwrap();
    ...
    <SectionBox className="space-y-6">
      <form onSubmit={handleSubmit} >
        <div style={{display:"flex",justifyContent:"space-between"}}>
                  <h2 className="text-xl font-bold">{t("add_attendance_rule")}</h2>
...
<div>
  ...
  <Select
    ...
    onChange={(selected) =>
      setFormData(prev => ({ ...prev, shift_time: selected?.value }))
    }
...
export default AddAttendanceRule;
