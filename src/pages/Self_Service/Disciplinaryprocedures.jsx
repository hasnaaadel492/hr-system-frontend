...
import {  useGetAllDisciplinaryproceduresDataQuery } from '../../api/Self_ServicesSlice';

const Disciplinaryprocedures = () => {
   const { data,  isLoading,refetch } = useGetAllDisciplinaryproceduresDataQuery();
        ...
        >
          <thead>
            <tr className="opacity-50 text-[20px]">
              <th className="font-400" style={{ padding: '5px' }}>تاريخ الإجراء</th>
              <th className="font-400" style={{ padding: '5px' }}>نوع الإجراء</th>
              <th className="font-400" style={{ padding: '5px' }}>وصف الإجراء</th>
          ...
          <tbody>
            {data?.data?.map((item) => (
              ...
              >
                ...
                <td style={{ padding: '5px' }}>{item?.action_type}</td>
  ...
  {item?.status === "done" ? (
    ...
    <>
      <span className="bg-[#F9C4C4] text-[#FB0408] opacity-50 w-fit rounded-xl p-2">
        {item?.status}
      </span>
    </>
  )}
</td>
                <td style={{ padding: '5px' }}>{item?.recorder?.name}</td>
