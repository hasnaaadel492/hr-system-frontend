...
import { useGetAllEvalutionsDataQuery } from "../../api/Self_ServicesSlice";

const ReviewtheevaluationDeta = () => {
       const { data,  isLoading,refetch } = useGetAllEvalutionsDataQuery();
        ...
        >
          <thead>
            <tr className="opacity-50 text-[23px]">
              ...
              <th className="font-400" style={{ padding: '5px' }}>تاريخ التقييم</th>
              <th className="font-400" style={{ padding: '5px' }}>نوع التقييم</th>
              <th className="font-400" style={{ padding: '5px' }}>الدرجة  </th>
              <th className="font-400" style={{ padding: '5px' }}>التقييم من قبل</th>
          ...
          <tbody>
            {data?.data?.map((item, index) => (
              ...
              >
                ...
                <td style={{ padding: '5px' }}>{item?.evaluation_date}</td>
                <td style={{ padding: '5px' }}>{item?.evaluation_type}</td>
                <td style={{ padding: '5px' }}>{item?.score}</td>
                <td style={{ padding: '5px' }}>{item?.evaluator}</td>
...
export default ReviewtheevaluationDeta;
