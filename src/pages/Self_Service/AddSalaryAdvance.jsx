...
const AddSalaryAdvance = () => {
    ...
    const schema = z.object({
      loan_amount: z
        .number()
        .positive('المبلغ يجب أن يكون رقمًا موجبًا.'),
        date_loan: z.string().nonempty('التاريخ مطلوب.'),
    ...
    const handleChange = (e) => {
      ...
      if (type === 'checkbox') {
        ...
        setFormData((prev) => ({
          ...prev,
          [name]: type === 'number' ? Number(value) : value, // Parse number inputs
    ...
    const handleSubmit = async (e) => {
      ...
      data.append('loan_amount', formData.loan_amount);
      data.append('date_loan', formData.date_loan);
  ...
  <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
    ...
    <li>
      ...
      <IoIosArrowBack className='mt-0.5 text-[#6F6C90]'/>
        <a className="ms-1 text-2xl font-medium text-[#6F6C90] hover:text-blue-600  ">طلب سلفة</a>
      </div>
    </li>
...
<div className="bg-[#FAF8F8]  p-10  shadow-xl w-[65%] rounded-lg mx-auto">
  ...
  <h5 className="text-[#170F49] text-2xl font-bold">طلب سلفة</h5>
  <p className="text-[#6F6C90] text-lg mb-6">إضف معلومات المطلوبة لطلب سلفة جديدة</p>
    </div>
  
  <form className="space-y-6  w-[50%] rounded-lg mx-auto"  onSubmit={handleSubmit}>
    {/* Leave Type */}
    <div className="flex flex-col justify-between gap-4">
        <label className='text-[#170F49] text-[16px]'  htmlFor="loan_amount">قيمة السلفة</label>
      <input
        type="number"
        id="loan_amount"
        name="loan_amount"
    ...
    {/* Date Range */}
    <div className="flex flex-col justify-between gap-4">
        <label htmlFor="date_loan">التاريخ</label>
  ...
  <div className="relative w-full">
    ...
    />
      {errors.date_loan && (
            <p className="text-red-500 text-sm">{errors.date_loan}</p>
          )}
  </div>
  ...
  <div className="flex bg-white px-2 py-8 shadow-xl rounded-3xl relative flex-col items-center space-y-2">
    {!formData.file_path ? (
      <>
        {/* SVG shown when no file is selected */}
        <div>
          <svg
            width="41"
            height="35"
          ...
          >
            <path
              d="M30.7056 9.20921C30.1401 9.09646 29.6772 8.74283 29.438 8.23717C26.7901 2.64921 20.652 -0.475332 14.5174 0.655584C8.92775 1.68058 4.54588 6.14275 3.61142 11.7615C3.33467 13.4203 3.35517 15.0808 3.66779 16.6985C3.77029 17.2264 3.54308 17.8141 3.07671 18.2378C1.12067 20.0161 0 22.5496 0 25.1907C0 30.3703 4.21446 34.5865 9.39583 34.5865H28.1875C35.2532 34.5865 41 28.8397 41 21.774C41 15.6838 36.6711 10.3982 30.7056 9.20921ZM25.8317 20.4193C25.4986 20.7524 25.0613 20.9198 24.6239 20.9198C24.1866 20.9198 23.7493 20.7524 23.4161 20.4193L20.5 17.5032V26.0448C20.5 26.9895 19.7347 27.7532 18.7917 27.7532C17.8487 27.7532 17.0833 26.9895 17.0833 26.0448V17.5032L14.1672 20.4193C13.4993 21.0873 12.4196 21.0873 11.7516 20.4193C11.0837 19.7513 11.0837 18.6717 11.7516 18.0037L16.3761 13.3793C17.0355 12.7198 17.9016 12.3884 18.7677 12.3833L18.7917 12.3782L18.8156 12.3833C19.6834 12.3884 20.5478 12.7198 21.2073 13.3793L25.8317 18.0037C26.4997 18.6717 26.4997 19.7513 25.8317 20.4193Z"
              fill="grey"
            />
          </svg>
        </div>
        <p className="text-[#170F49] text-[12px] font-thin">
          Drop your file or upload from your device
        </p>
      </>
    ) : (
      <>
        {/* File name and delete button when a file is selected */}
        <p className="text-green-500 text-sm mt-2">{formData.file_path.name}</p>
        <button
          type="button"
          onClick={() => setFormData((prev) => ({ ...prev, file_path: null }))}
